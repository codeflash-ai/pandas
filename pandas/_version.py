...
def plus_or_dot(pieces) -> str:
    ...
    ct = pieces.get("closest-tag")
    # Avoid default param, check None and skip get("") overhead
    if ct and "+" in ct:
        return "."
    return "+"
...
def render_pep440_branch(pieces):
    ...
    short = pieces["short"]
    dirty = pieces["dirty"]
    # Avoid repeated global/local lookups and dict access
    if closest_tag:
        rendered = closest_tag
        if distance or dirty:
            if branch != "master":
                rendered += ".dev0"
            # concat intermediate values in a single operation for CPython string optimizer
            pdot = plus_or_dot(pieces)
            rendered = f"{rendered}{pdot}{distance}.g{short}"
            if dirty:
        ...
        if branch != "master":
            rendered += ".dev0"
        # Use f-string for all static concatenation in one operation
        rendered = f"{rendered}+untagged.{distance}.g{short}"
        if dirty:
            rendered = f"{rendered}.dirty"
    return rendered
