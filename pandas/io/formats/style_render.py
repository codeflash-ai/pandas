...
def _parse_latex_cell_styles(
    ...
    for command, options in reversed(latex_styles):
        options_str = options if isinstance(options, str) else str(options)
        found_arg = None
        ...
        if found_arg is None:
            ...
            parsed_opt = _parse_latex_options_strip(value=options, arg=found_arg)
            if found_arg == "--wrap":
                display_value = f"{{\\{command}{parsed_opt} {display_value}}}"
            elif found_arg == "--nowrap":
                display_value = f"\\{command}{parsed_opt} {display_value}"
            elif found_arg == "--lwrap":
                display_value = f"{{\\{command}{parsed_opt}}} {display_value}"
            elif found_arg == "--rwrap":
                display_value = f"\\{command}{parsed_opt}{{{display_value}}}"
            elif found_arg == "--dwrap":
                display_value = f"{{\\{command}{parsed_opt}}}{{{display_value}}}"
