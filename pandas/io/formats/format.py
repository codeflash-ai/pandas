...
def _trim_zeros_float(
    ...
    # Inline and optimize number matching using the compiled regex
    def is_number_with_decimal(x: str) -> bool:
        return bool(number_regex.match(x))

    # Convert input to a list if necessary only once, for repeated indexing/slicing
    if isinstance(str_floats, list):
        trimmed = str_floats[:]
    else:
        trimmed = list(str_floats)

    # Cache indices of decimal numbers once to avoid recomputation in loops
    indices_with_decimal = [
        i for i, x in enumerate(trimmed) if is_number_with_decimal(x)
    ]
    # If no numbers fit the regex, just return as per original logic
    if not indices_with_decimal:
        return trimmed

    n = len(trimmed)
    keep_trimming = True
    while keep_trimming:
        # Check only decimal numbers if they all end with "0"
        keep_trimming = True
        for i in indices_with_decimal:
            if not trimmed[i].endswith("0"):
                keep_trimming = False
                break
        if keep_trimming:
            # Only alter target elements
            for i in indices_with_decimal:
                trimmed[i] = trimmed[i][:-1]

    # Ensure at least one zero after decimal if ends directly with decimal
    # *Precompute* the result to minimize function call/regex checks
    result = trimmed
    for i in indices_with_decimal:
        x = result[i]
        if x.endswith(decimal):
            result[i] = x + "0"
    return result
