...
def _highlight_value(data: DataFrame | Series, op: str, props: str) -> np.ndarray:
    ...
    cond = data == value
    # Optimize: avoid .where overhead by setting False where notna(cond) is False
    notna_mask = pd.notna(cond)
    # For DataFrame/Series, cond is same type, so use cond.values if DF/Series
    # This uses fast numpy boolean masking without reindex
    # We preserve cond's dtype and shape by notna_mask
    cond = np.where(notna_mask, cond, False)
