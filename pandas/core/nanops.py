...
def check_below_min_count(
    ...
    if min_count > 0:
        if mask is None:
            ...
            # Avoid np.prod for single or two dimensions, use multiplication
            # for speed
            if len(shape) == 1:
                non_nulls = shape[0]
            elif len(shape) == 2:
                non_nulls = shape[0] * shape[1]
            else:
                # For >=3 dimensions fallback to np.prod
                non_nulls = 1
                for dim in shape:
                    non_nulls *= dim
            # np.prod is avoided for common case, saves overhead
        else:
            # mask.size is simple integer, mask.sum() is costly for large arrays.
            # Use np.count_nonzero if there are far more False values (rare)
            # but keep the logic to subtract the mask.sum() (number of True = missing)
            missing_count = np.count_nonzero(mask)
            non_nulls = mask.size - missing_count
