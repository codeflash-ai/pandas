...
def _is_uniform_join_units(join_units: list[JoinUnit]) -> bool:
    ...
    """
    first_blk = join_units[0].block
    first_type = type(first_blk)
    first_dtype = first_blk.dtype
    ...
    for ju in join_units:
        if type(ju.block) is not first_type:
            return False

    # Check dtype uniformity (with kind-in check)
    for ju in join_units:
        if ju.block.dtype == first_dtype:
            continue
        if ju.block.dtype.kind in "iub":
            continue
        return False

    # Check for missing values (type upcasts), unless extension dtype
    for ju in join_units:
        if ju.is_na and not ju.block.is_extension:
            return False
