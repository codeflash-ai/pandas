...
class NDFrame(PandasObject, indexing.IndexingMixin):
    ...
    def _repr_latex_(self):
        ...
        """
        styler_render_repr = config.get_option("styler.render.repr")
        if styler_render_repr == "latex":
    ...
    def to_latex(
        ...
        *,
        ...
        # Optimize option reads: only call config.get_option for each key once
        option_keys = (
        ...
        option_values = {}
        # Only fetch options if any are needed (i.e., only on None)
        need_longtable = longtable is None
        need_escape = escape is None
        need_multicolumn = multicolumn is None
        need_multicolumn_format = multicolumn_format is None
        need_multirow = multirow is None
        if any([need_longtable, need_escape, need_multicolumn, need_multicolumn_format, need_multirow]):
            for key in option_keys:
                option_values[key] = config.get_option(key)

        if need_longtable:
            longtable = option_values["styler.latex.environment"] == "longtable"
        if need_escape:
            escape = option_values["styler.format.escape"] == "latex"
        if need_multicolumn:
            multicolumn = option_values["styler.sparse.columns"]
        if need_multicolumn_format:
            multicolumn_format = option_values["styler.latex.multicol_align"]
        if need_multirow:
            multirow = option_values["styler.sparse.index"]
